...
优惠券由**每个租户（店铺）独立发行**，买家可在本店或默认商城领取使用。系统支持满减、折扣、无门槛直减与**免邮券**四种类型，券名与说明支持多语言。

### 11.1 优惠券类型

| 类型 | 含义 | 示例 |
|------|------|------|
| `FIXED` | 满额立减（需满足门槛） | 满 100 减 20 |
| `PERCENT` | 按折扣比例优惠 | 8.5 折 |
| `FULL` | 无门槛直减 | 立减 10 元 |
| `FREE_SHIPPING` | 免配送费 | 全场免邮券 |

### 11.2 发行与使用范围

- **每租户发行**：优惠券归属发行店铺（`CouponTemplate.shopId`），管理员只能管理**本店发行的券**，无法查看/编辑其他店铺的券（属店权限隔离）。
- **使用范围**：券可用于**本店商品**或**默认租户（默认商城）**。在默认商城使用时，仅当订单包含**本店商品行**（`product.customFields.shopId === 券.shopId`）才可核销，且折扣只作用于本店商品行；无本店商品行时报 `COUPON_SCOPE_MISMATCH`。
- **多语言**：券名与说明为 `LocalizedText`（`string | Record<LanguageCode, string>`），按会话语言经 `localizeText` 逐级回退显示（当前 locale → en → 首个值 → 占位）。

### 11.3 买家端：领券中心与我的券包

商城「优惠券」页提供**领券中心**（浏览并领取可领券）与**我的券包**（按未使用/已使用/已过期查看已领券）两个 Tab。

![领券中心](assets/coupon-centre.png)

> 领券中心：展示全场免邮券（FREE_SHIPPING）与满减券（FIXED），券名按当前语言显示中文。

![我的券包](assets/coupon-wallet.png)

> 我的券包：未使用 / 已使用 / 已过期状态筛选。

### 11.4 管理端：发券与券管理

后台「优惠券管理」入口可**创建 / 编辑 / 启停 / 删除**本店优惠券，配置类型、面额、门槛、有效期、限量与多语言名称。

![优惠券管理列表](assets/coupon-admin-list.png)

> 优惠券管理列表：分页展示本店发行的券，支持启停、编辑、删除。

![优惠券编辑](assets/coupon-admin-edit.png)

> 优惠券编辑：选择类型（含 FREE_SHIPPING 免邮券）、配置面额/门槛/有效期，多语言名称与说明。

### 11.5 后端接口

| 类型 | 入口 | 说明 |
|------|------|------|
| Query | `couponCentre` | 领券中心（默认商城追加列出含本店商品的可领租户券） |
| Query | `myCoupons` | 我的券包（按状态筛选，券名/说明已本地化） |
| Mutation | `claimCoupon` | 领取优惠券 |
| Mutation | `applyCouponToOrder` / `clearCouponFromOrder` | 结算选券 / 取消选券（选券前校验范围） |
| Admin | `couponTemplates` / `createCouponTemplate` / `updateCouponTemplate` | 发券管理（按属店权限过滤与校验） |

> 相关实现位于 coupon-plugin：`coupon-promotion-condition.ts`（折扣计算 + 跨渠道范围判定）、`coupon-template.entity.ts`（shopId + LocalizedText）、`coupon.service.ts`（发行/领券/选券/属店校验）、`localize.ts`（多语言回退）。

## 12. 结算页逐箱结算：商户分账与地址字段

结算页按 **商家分组 → 逐箱卡片** 组织，多商家/多配送档案各自成箱；每箱展示租户名、本箱商品、本箱优惠与小计，支付方式=各被选箱配送档案支付方式的交集。提交结算前下方展示**「分账明细」**，按商户汇总应计入金额与**应付款总额**。

### 12.1 分账明细与应付款总额

合并支付（在线 / 货到付款）时页面显式提示**「将合并为 1 单支付」**，并按商户逐行列出分账金额及应付款总额；实收按商户分账记账。

![结算页分账明细](assets/checkout-merchant-split.png)

> 结算页：分账明细（校验商户分账金额）+ 应付款总额 + 合并 1 单提示。各箱商品小计、运费逐行核算。

### 12.2 地址字段对齐（邮编物流必填 + 公司选填）

物流配送地址表单字段严格对齐 Vendure `Address`：**收货人 / 电话 / 省市区 / 详细地址 / 邮编必填**，其中 **邮政编码为物流必填**（自提无地址、不采集邮编）；**公司、地址2（楼栋/楼层/门牌）为选填**。空表提交会本地校验拦截。

![地址表单：公司选填 + 邮编必填](assets/address-company-postal.png)

> 地址表单：`公司（选填）`、`地址2（选填）`，`邮政编码` 为必填（物流）。

> **说明**：自提订单不采集收货地址因此无邮编。合并支付时各商户分账按**台账**记账（在线支付=支付成功回调入账；货到付款=签收后入账）；通道级分账（各商户开通支付通道服务商分账）属后续 Roadmap。

### 12.3 逐箱卡片：外观与商品行级选择

结算页主体由**统一逐箱卡片渲染器**（`CheckoutPerBoxList`）按「租户分区 → 箱卡」组装：每箱卡片含 **租户名 + 配送档案名 + 箱型标签（物流/自提）**，箱卡内列出商品行（**缩略图 / 名称 / 规格名+SKU / 单价 / 数量步进 / 行小计 / 删除**）、配送方式（物流箱）或自提点（自提箱）、本箱商品小计与优惠券行。cn / jd 版式共用同一渲染器，仅各自外围（收货地址块 / 联系人卡 / 支付块 / 结算栏 / 分账汇总）不同，旧京东版式保留为 `jd-legacy` 可回退。

![逐箱卡片总览](assets/perbox-card-overview.png)

> 逐箱卡片总览：整箱勾选 + 商品行级勾选 + 数量步进 + 删除；物流箱含配送方式，箱卡底部为商品小计与优惠券行。

**商品行级选择**：箱头勾选=整箱全选/反选；每行独立勾选=部分结算；`−`/`＋` 步进数量（不可为 0）；行尾「删除」为兜底。已选项驱动整页实时刷新「商品小计 / 本箱金额 / 商户分账 / 应付款总额 / 已选 N 件」。局部结算成功后，**未选行按量回流购物车**（幂等 `AddItemToOrder`），避免丢单。

![部分结算：仅已选项计入金额](assets/perbox-card-line-selection.png)

> 部分结算：取消勾选某商品行后，商品小计 / 本箱金额 / 商户分账 / 应付款总额均按已选项重算（此处未选 ¥676.87 行，实付 ¥145.77），未选行结算后回流购物车。

### 12.4 支付交集与分箱支付

**支付方式 = 各被选箱配送档案支付方式白名单的交集**。交集非空 → **合并 1 单统一支付**（页面显式提示「将合并为 1 单支付」）；交集为空或余额不足 → **按箱分组单选、分箱支付**（每箱独立一单，箱内方式互斥）。顺带说明：逐箱/部分结算时的**商户分账按「已选箱」租户聚合**（会实收一致），地址字段严格对齐 Vendure `Address`（见 §12.2，邮编物流必填 / 公司、地址2 选填 / 邮箱客户级选填）。

![支付区：合并 1 单支付 + 固定聚合码收款](assets/perbox-card-payment.png)

> 支付区：合并 1 单支付提示 + 支付方式（本箱唯一方式自动锁定）。多箱支付方式不一致时改为按箱分组单选、分箱支付。

### 12.5 优惠券切换与版式回退

整单使用一张券：在箱卡「优惠券」行打开**券选择抽屉**，选中后应用到整单并即时刷新各箱优惠运费 / 折扣。未登录访客提示「登录后可选用券包优惠券」。`checkout-config` 支持 `cn`（中国本地化，默认）/ `jd`（京东新版）/ `jd-legacy`（旧京东回退）切换，模板或配置故障时可逐级回退，不影响下单。

![优惠券行与登录提示](assets/perbox-card-coupon.png)

> 优惠券：整单一券，箱卡「优惠券」行进入抽屉；访客提示登录后可用。
