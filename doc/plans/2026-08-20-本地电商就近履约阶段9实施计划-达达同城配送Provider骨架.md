...
- [ ] **Step 2：package.json 新增 test 脚本**
...
- [ ] **Step 1：修改 applyStatusEvent 兜底按 thirdPartyNo 定位**
...
- [ ] **Step 3：tsc 类型检查（禁 pnpm build/install）**
...
- [ ] **Step 1：重建 delivery-gateway-plugin 产物（禁 pnpm build/install）**
...
1. [ ] `DadaDeliveryProvider` 实现四方法，`code='dada'`，经 `registerProvider` 注册。
2. [ ] 签名单测固化 `signature = MD5(app_secret + 升序拼接串 + app_secret)`（固定向量 `69B39F...`）。
3. [ ] 状态码映射 1/2/3/4/5/7/10/1000 → pending/accepted/pickup/delivered/cancelled/exception 全表单测覆盖。
4. [ ] webhook 入站无账号 e2e：验签 → 状态机推进 → OrderPackage 回写 → `{"status":"ok"}`；验签失败 401；client_id 兜底定位；终态幂等。
5. [ ] `environment: sandbox/production` 仅换 baseUrl（`ENV_BASE_URL`）。
6. [ ] 无凭据出站不真实请求（quote 失败返回 available:false）；占位配置可注册；既有 9 套回归全绿。
