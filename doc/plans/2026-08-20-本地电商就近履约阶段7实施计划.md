...
// 注：In 不从 @vendure/core 导出，须从 typeorm 导入（core 自身 helpers 同款写法）
...
async getMyOrderPackages(
    ...
    // 4. 富化：join OrderLine → productVariant → product → translations 取 productName/sku
    //    注：Vendure OrderLine 实体关系名为 productVariant（非 variant），此处用 productVariant 编译通过
