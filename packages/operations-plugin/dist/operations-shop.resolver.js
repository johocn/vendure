"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.OperationsShopResolver = void 0;
// e:\code\vendure\packages\operations-plugin\src\operations-shop.resolver.ts
const graphql_1 = require("@nestjs/graphql");
const core_1 = require("@vendure/core");
const content_service_1 = require("./content.service");
/**
 * @description
 * Operations Shop API Resolver (schema-first mode).
 * Only exposes public content queries; dashboard is admin-only.
 *
 * Note: @Allow(Permission.Public) is REQUIRED for public access.
 * Vendure's default behavior when @Allow is not set is to deny access.
 */
let OperationsShopResolver = class OperationsShopResolver {
    constructor(contentService) {
        this.contentService = contentService;
    }
    async publishedContent(ctx, type, position) {
        return this.contentService.findPublishedContentItems(ctx, { type, position });
    }
};
exports.OperationsShopResolver = OperationsShopResolver;
__decorate([
    (0, graphql_1.Query)(),
    (0, core_1.Allow)(core_1.Permission.Public),
    __param(0, (0, core_1.Ctx)()),
    __param(1, (0, graphql_1.Args)({ name: 'type', type: () => String, nullable: true })),
    __param(2, (0, graphql_1.Args)({ name: 'position', type: () => String, nullable: true })),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [core_1.RequestContext, String, String]),
    __metadata("design:returntype", Promise)
], OperationsShopResolver.prototype, "publishedContent", null);
exports.OperationsShopResolver = OperationsShopResolver = __decorate([
    (0, graphql_1.Resolver)(),
    __metadata("design:paramtypes", [content_service_1.ContentService])
], OperationsShopResolver);
