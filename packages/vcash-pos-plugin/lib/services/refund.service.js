...
let RefundService = class RefundService {
    constructor(connection, transactionalConnection, orderService) {
    ...
    async createRefund(ctx, input) {
        ...
        if (!order) {
            throw new core_1.UserInputError(`订单 ${input.originalOrderId} 不存在`);
        }
        ...
        if (!refundableStates.includes(order.state)) {
            throw new core_1.UserInputError(`订单状态 ${order.state} 不支持退款（需 PaymentSettled 等已完成状态）`);
        }
        // 2. 校验 Payment 属于该 Order
        const payment = (_a = order.payments) === null || _a === void 0 ? void 0 : _a.find(p => String(p.id) === String(input.paymentId));
        if (!payment) {
            throw new core_1.UserInputError(`Payment ${input.paymentId} 不属于订单 ${input.originalOrderId}`);
        }
        ...
        const refundResult = await this.orderService.refundOrder(ctx, {
            ...
            amount: input.amount,
            reason: (_b = input.reason) !== null && _b !== void 0 ? _b : 'POS 退货退款',
            shipping: 0,
            adjustment: 0,
        });
        if ('errorCode' in refundResult) {
            throw new core_1.UserInputError(`创建退款失败: ${refundResult.message}`);
        }
        ...
        if (payment.method === 'cash') {
            ...
            // 聚合码：标记需人工退款
            await this.connection.getRepository(core_1.Refund).update(refundResult.id, {
                metadata: Object.assign(Object.assign({}, ((_c = refundResult.metadata) !== null && _c !== void 0 ? _c : {})), { needsManualRefund: true, originalPaymentMethod: 'aggregate' }),
            });
        }
        // 5. 返回最新 Refund
        return this.connection.getRepository(core_1.Refund).findOne({
     ...
     */
    async settleManualRefund(ctx, input) {
        const refund = await this.connection.getRepository(core_1.Refund).findOne({
            where: { id: input.refundId },
        });
        if (!refund) {
            throw new core_1.UserInputError(`退款单 ${input.refundId} 不存在`);
        }
        if (refund.state !== 'Pending') {
            throw new core_1.UserInputError(`退款单 ${input.refundId} 状态为 ${refund.state}，无法结算（需 Pending）`);
        }
     ...
     */
    async findRefundsBySession(sessionId) {
        var _a;
        // 查找 order 表的 posSessionId 列名
        const orderColumns = (await this.connection.query('PRAGMA table_info("order")'));
        const sessionCol = (_a = orderColumns.find(c => c.name.toLowerCase().replace(/_/g, '').includes('possessionid'))) === null || _a === void 0 ? void 0 : _a.name;
        if (!sessionCol)
            return [];
        // 查找属于该班次的 Order IDs
        ...
        if (refundIds.length === 0)
            return [];
        return this.connection.getRepository(core_1.Refund).find({
            where: { id: (0, typeorm_2.In)(refundIds) },
            relations: ['payment'],
        });
    }
};
