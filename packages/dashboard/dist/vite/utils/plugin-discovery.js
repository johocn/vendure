...
export async function discoverPlugins({ vendureConfigPath, transformTsConfigPathMappings, logger, outputPath, pluginPackageScanner, }) {
    var _a;
    const plugins = [];
    const nodeModulesRoot = (_a = pluginPackageScanner === null || pluginPackageScanner === void 0 ? void 0 : pluginPackageScanner.nodeModulesRoot) !== null && _a !== void 0 ? _a : guessNodeModulesRoot(vendureConfigPath, logger);
    // Analyze source files to find local plugins and package imports
    const { localPluginLocations, packageImports } = await analyzeSourceFiles(vendureConfigPath, nodeModulesRoot, logger, transformTsConfigPathMappings);
    ...
    logger.debug(`[discoverPlugins] Expanded to ${expandedImports.length} packages: ${JSON.stringify(expandedImports, null, 2)}`);
    const localPluginLibGlobs = [];
        ...
        try {
            ...
            walkSimple(ast, {
                CallExpression(node) {
                    ...
                    const calleeName = (_a = callee.name) !== null && _a !== void 0 ? _a : (callee.type === 'MemberExpression' && ((_b = callee.property) === null || _b === void 0 ? void 0 : _b.name) === '__decorate'
                        ? '__decorate'
                        : undefined);
                                ...
                                for (const prop of props) {
                                    if (prop.key.name === 'dashboard') {
                                        if (prop.value.type === 'Literal') {
                                            ...
                                            const locationProp = (_c = prop.value.properties) === null || _c === void 0 ? void 0 : _c.find((p) => { var _a; return ((_a = p.key) === null || _a === void 0 ? void 0 : _a.name) === 'location'; });
                                            if ((locationProp === null || locationProp === void 0 ? void 0 : locationProp.value.type) === 'Literal') {
 ...
 */
async function expandPackageImports(packageImports, nodeModulesRoot, logger) {
    var _a;
    ...
    for (const pkg of packageImports) {
        ...
        try {
            ...
            logger.debug(`[expandPackageImports] Could not read package.json for ${pkg}: ${e instanceof Error ? e.message : String(e)}`);
            continue;
        }
        const deps = Object.keys((_a = pkgJson.dependencies) !== null && _a !== void 0 ? _a : {});
        await Promise.all(deps.map(async (dep) => {
            if (expanded.has(dep) || dep.startsWith('@vendure/'))
                return;
            let depPkgJson;
            try {
                ...
                logger.debug(`[expandPackageImports] Could not read package.json for transitive dep ${dep} (via ${pkg})`);
                return;
            }
            const allDeps = Object.assign(Object.assign({}, depPkgJson.dependencies), depPkgJson.peerDependencies);
            if ('@vendure/core' in allDeps) {
                logger.debug(`[expandPackageImports] Found transitive Vendure package: ${dep} (via ${pkg})`);
                expanded.add(dep);
            }
        }));
    }
...
export async function findVendurePluginFiles({ outputPath, vendureConfigPath, logger, nodeModulesRoot: providedNodeModulesRoot, packageGlobs, }) {
    ...
    const patterns = [
        ...
        ...packageGlobs.map(pattern => path.isAbsolute(pattern) ? pattern : path.join(nodeModulesRoot, pattern)),
    ].map(p => p.replace(/\\/g, '/'));
