 ...
 */
export declare const COD_PAYMENT_CODES: ReadonlyArray<string>;
 ...
 */
export declare function merchantSettlementStatus(method: string, codPaymentCodes: ReadonlyArray<string>): MerchantSettlementStatus;
/**
 * 商户分账台账服务：在结算拆单时，将一笔已结算订单按「商户（租户）」写入台账。
 * 采用 OrderBoxService.computeMerchantSplit 的分账口径（per tenant，合算该订单内各箱 subtotal）。
 */
export declare class MerchantSettlementService {
    private orderBoxService;
    private connection;
    constructor(orderBoxService: OrderBoxService, connection: TransactionalConnection);
    /**
     ...
     */
    recordOrderSettlement(ctx: RequestContext, order: Order, opts: {
        method: string;
        codPaymentCodes: string[];
    }): Promise<void>;
}
