            ...
            if (result && typeof result === 'object') {
                ...
                if (customer) {
                    ...
                    if (finalInviteCode) {
                        ...
                        try {
                            await this.distributionService?.apply(
                                ctx,
                                customer.id as any,
                                String(finalInviteCode),
                                ownInviteCode ? String(ownInviteCode) : undefined,
                            );
                        } catch (e: any) {
                    ...
                    if (ownInviteCode && !cf.referralCode) {
                        try {
                            await this.customerService.update(ctx, {
                                id: customer.id as any,
                                customFields: { referralCode: String(ownInviteCode) as any } as any,
                            });
                        } catch (e: any) {
                            Logger.warn(`Failed to persist own referralCode: ${e.message}`, loggerCtx);
                        }
                    }
                }
            }
