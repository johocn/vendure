...
const defaultResolver = hostname => (0, promises_1.lookup)(hostname, { all: true, verbatim: true });
/**
 * @description
 ...
 * override.
 */
async function assertPublicUrl(urlString, options = {}) {
    var _a;
    let url;
    try {
        ...
        vendure_logger_1.Logger.warn(`Refusing to fetch invalid URL: ${safeForLog(urlString)}`, loggerCtx);
        throw new errors_1.InternalServerError(GENERIC_INVALID_URL_MESSAGE);
    }
    if (url.protocol !== 'http:' && url.protocol !== 'https:') {
        vendure_logger_1.Logger.warn(`Refusing to fetch URL with unsupported protocol "${url.protocol}": ${safeForLog(urlString)}`, loggerCtx);
        throw new errors_1.InternalServerError(GENERIC_INVALID_URL_MESSAGE);
    }
    ...
    const resolve = (_a = options.resolver) !== null && _a !== void 0 ? _a : defaultResolver;
    let addresses;
    ...
    if (literalFamily === 4 || literalFamily === 6) {
        ...
        try {
            ...
            vendure_logger_1.Logger.warn(`Refusing to fetch ${safeForLog(urlString)}: DNS lookup failed (${safeForLog(message)})`, loggerCtx);
            throw new errors_1.InternalServerError(GENERIC_REJECTION_MESSAGE);
        }
    }
    else {
        vendure_logger_1.Logger.warn(`Refusing to fetch URL with empty hostname: ${safeForLog(urlString)}`, loggerCtx);
        throw new errors_1.InternalServerError(GENERIC_INVALID_URL_MESSAGE);
    }
    for (const { address, family } of addresses) {
        const normalisedFamily = family === 6 ? 6 : 4;
        if (isBlocked(address, normalisedFamily)) {
            vendure_logger_1.Logger.warn(`Refusing to fetch ${safeForLog(urlString)}: resolves to non-public address ${address}. ` +
                'Set `assetImportStrategy: new DefaultAssetImportStrategy({ allowPrivateNetworks: true })` ' +
                'in your VendureConfig if this is intentional (e.g. a trusted internal asset server).', loggerCtx);
            throw new errors_1.InternalServerError(GENERIC_REJECTION_MESSAGE);
        }
    }
...
//# sourceMappingURL=assert-public-url.js.map