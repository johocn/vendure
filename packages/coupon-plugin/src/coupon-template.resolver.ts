 ...
 */
@Resolver('CouponTemplate')
export class CouponTemplateResolver {
    @ResolveField('name')
    name(@Parent() template: CouponTemplate, @Ctx() ctx: RequestContext): string {
        return localizeText(template.name as any, ctx.languageCode, '');
    }

    @ResolveField('description')
    description(@Parent() template: CouponTemplate, @Ctx() ctx: RequestContext): string | null {
        const v = (template as any).description;
        if (v == null) {
            return null;
        }
